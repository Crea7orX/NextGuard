...
export function useAdoptPendingDeviceMutation({ id }: Props) {
  const queryClient = useQueryClient();

  return useMutation<PendingDeviceResponse, AxiosError>({
    mutationKey: ["PendingDevices", "Adopt"],
    mutationFn: async () =>
      (
        await axiosInstance.post<SuccessResponse<PendingDeviceResponse>>(
          `/pending_devices/${id}/adopt`,
        )
      ).data.data,
    onSuccess: () =>
      void Promise.all([
